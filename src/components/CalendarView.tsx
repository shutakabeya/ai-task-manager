  ...
  if (tasks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">スケジュールがありません</h3>
        <p className="text-gray-500">タスクを作成して日時を設定すると、ここに表示されます</p>
      </div>
    )
  }
        ...
        <div className="grid grid-cols-7">
          {weekDates.map((date, index) => {
            ...
            return (
              <div
                key={index}
                className={`min-h-32 border-r border-gray-200 last:border-r-0 ${
                  isToday ? 'bg-blue-50' : ''
                }`}
              >
                {/* 日付 */}
                ...
                <div className="p-2 space-y-1">
                  {subtasks.map(({ task, subtask }) => (
                    <div
                      key={subtask.id}
                      className={`p-2 rounded text-xs text-white ${getCategoryColor(task.category, subtask.completed)} transition-all ${
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-600">仕事</span>
          </div>
          <div className="flex items-center space-x-2">
